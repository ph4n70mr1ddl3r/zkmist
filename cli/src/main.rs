...
mod tests {
    ...
    fn test_eip55_valid_checksum() {
        // Vitalik's address — known valid EIP-55 mixed-case
    ...
    fn test_eip55_all_lowercase_bypasses_checksum() {
        // All lowercase should be accepted without checksum validation
        let result = validate_address_checksum("0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaed");
        assert!(result.is_ok());
    }
    ...
    fn test_eip55_all_uppercase_bypasses_checksum() {
        // All uppercase should be accepted without checksum validation
        let result = validate_address_checksum("0x5AAEB6053F3E94C9B9A09F33669435E7EF1BEAED");
        assert!(result.is_ok());
    }
    ...
    fn test_eip55_no_prefix_still_works() {
        // Without 0x prefix, all-lowercase should still work
    ...
    fn test_eip55_invalid_checksum_rejected() {
        // Flip one letter — should fail EIP-55
        let result = validate_address_checksum("0x5aAeb6053F3E94C9b9A09f33669435E7Ef1BeAeD");
    ...
    fn test_eip55_rejects_zero_address() {
        // Zero address is parsed fine but should be caught at a higher level
    ...
    fn test_derive_address_prd_test_vector() {
        // PRD Appendix D: private key → 0xfcad0b19bb29d4674531d6f115237e16afce377c
        let key: [u8; 32] = [
    ...
    fn test_derive_address_rejects_overflow_key() {
        // Key >= secp256k1 order is invalid
        let key: [u8; 32] = [
    ...
    fn test_days_to_ymd_known_dates() {
        ...
        // 2027-01-01 = CLAIM_DEADLINE / 86400
    ...
    fn test_format_deadline_known_timestamp() {
        // 2024-07-20 12:30:45 UTC = 1721478645
  ...
  "files": [{}]
    ...
    fn test_manifest_root_rejects_wrong_length() {
        let manifest = Manifest {
            ...
            fee_threshold_eth: String::new(),
            total_qualified: 0,
            merkle_root: "0x0102".to_string(), // only 2 bytes
            merkle_tree_depth: 26,
            files: vec![],
        };
    ...
    fn test_load_eligibility_list_with_csv() {
        ...
        let csv_path = dir.path().join("addrs_00.csv");
        std::fs::write(
            &csv_path,
            "address\n0x0000000000000000000000000000000000000001\n0x0000000000000000000000000000000000000002\n",
        ).unwrap();

        // Temporarily override eligibility_dir — load_eligibility_list reads from
        // eligibility_dir() which uses dirs::home_dir(). We'll create the temp
        // files in the expected structure instead.
        ...
        // Read and parse manually to match load_eligibility_list logic
    ...
    fn test_proof_file_roundtrip() {
        let original = ProofFile {
            ...
            journal: "e".repeat(168), // 84 bytes = 168 hex chars
    ...
    fn test_proof_file_without_receipt() {
        ...
        // receipt_hex should be omitted (skip_serializing_if)
    ...
    //
    // Verify that the alloy sol! macro generates correct selectors and
    // encoding for the claim() function. This catches subtle encoding bugs
    // that would cause on-chain reverts.

    #[test]
    fn test_claim_abi_selector() {
        // claim(bytes,bytes,bytes32,address) selector = first 4 bytes of keccak256
        let _call = claimCall {
    ...
    fn test_claim_abi_encoding_roundtrip() {
        ...
        // First 4 bytes must be the function selector
    ...
    fn test_claim_abi_encoding_with_real_journal() {
        // Build a realistic 84-byte journal and verify it survives encoding
        let root: [u8; 32] = [
            0x1e, 0xaf, 0xd6, 0xf3, 0xb8, 0xf3, 0x0a, 0xf9, 0x49, 0xff, 0x54, 0x93, 0xe9, 0x10,
            0x28, 0x53, 0xa7, 0xc2, 0x2f, 0x8c, 0xff, 0xdc, 0xf0, 0x18, 0xda, 0xa3, 0x1d, 0x42,
            0x45, 0x79, 0x78, 0x44,
        ...
        // Verify journal layout matches Solidity expectations:
        // [0:32] = root, [32:64] = nullifier, [64:84] = recipient
    ...
    fn test_total_claims_call_encoding() {
        ...
        let encoded = call.abi_encode();
        // totalClaims() is a simple 0-argument view function
        // encoded should be exactly 4 bytes (selector only)
    ...
    fn test_total_claims_return_decoding() {
        // Simulate a return value of totalClaims = 42
    ...
    fn test_is_claimed_call_encoding() {
        ...
        let encoded = call.abi_encode();
        // selector (4) + bytes32 (32) = 36 bytes
    ...
    fn test_total_supply_return_decoding() {
        let supply = alloy::primitives::U256::from(1_000_000_000_000_000_000_000_000u128); // 1M tokens with 18 decimals
    ...
    fn test_journal_layout_84_bytes() {
        // Verify that the expected 84-byte journal decomposes correctly
        let root: [u8; 32] = [
            0x1e, 0xaf, 0xd6, 0xf3, 0xb8, 0xf3, 0x0a, 0xf9, 0x49, 0xff, 0x54, 0x93, 0xe9, 0x10,
            0x28, 0x53, 0xa7, 0xc2, 0x2f, 0x8c, 0xff, 0xdc, 0xf0, 0x18, 0xda, 0xa3, 0x1d, 0x42,
            0x45, 0x79, 0x78, 0x44,
        ...
        // Slice exactly as Solidity does
    ...
    fn test_constants() {
        ...
        // CLAIM_DEADLINE should be 2027-01-01 00:00:00 UTC
    ...
    fn test_read_private_key_from_file_too_short() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("key.txt");
        std::fs::write(&path, "0x0123").unwrap();
    ...
    fn test_read_private_key_from_file_invalid_hex() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("key.txt");
        std::fs::write(&path, "0x".to_string() + &"Z".repeat(64)).unwrap();
        let err = read_private_key_from_file(path.to_str().unwrap()).unwrap_err();
        assert!(err.contains("Invalid hex"));
    }
    ...
    fn test_nullifier_matches_merkle_tree_lib() {
        ...
        let mut hasher = ark_poseidon_hasher(2).unwrap(); // CLI hasher
        let cli_nullifier = compute_nullifier(&key, &mut hasher);

        // Must match PRD test vector
        assert_eq!(
