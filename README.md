...
> - ✅ `Halo2Verifier.sol` + `Halo2VerifyingKey.sol` are regenerated at k=23 (0x17) with real fixed + permutation commitments (`gen-production-verifier --emit` against the pinned PSE SRS). The earlier k=21 placeholder is retired; the readiness checker confirms `VK k=23 == prover CIRCUIT_K`. **Remaining:** exercise the real-KZG → on-chain round-trip (`cargo run --release -p zkmist-cli -- gen-roundtrip-fixture` → `RUN_REAL_ROUNDTRIP=1 forge test --match-contract RealRoundtrip`), confirm the SRS transcript provenance, and complete the external audit (see [DEPLOYMENT.md](./DEPLOYMENT.md) Phase 3).
