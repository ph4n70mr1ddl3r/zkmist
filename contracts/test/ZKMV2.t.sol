...
contract ZKMV2Test is Test {
    ...
    address constant VERIFIER = address(0x2);
    ...
    function test_airdrop_deploy() public {
        airdrop = new ZKMAirdropV2(address(token), VERIFIER, MERKLE_ROOT);
        assertEq(address(airdrop.token()), address(token));
        assertEq(address(airdrop.verifier()), VERIFIER);
    ...
    function test_deploy_ordering() public {
        ...
        ZKMAirdropV2 a = new ZKMAirdropV2(address(t), VERIFIER, MERKLE_ROOT);
